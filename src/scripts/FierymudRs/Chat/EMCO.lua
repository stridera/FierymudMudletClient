...
--
-- Adapted from demonnic/EMCO 2.9.0 for the FierymudRs Mudlet
-- package. Two changes from upstream: `EMCO` is a global (not
-- `local`) because Mudlet script bodies aren't loaded via
-- `require()`, and the `return EMCO` at end-of-file is dropped
-- for the same reason. The `loggingconsole` require near the top
-- is also replaced with `local LC = nil` — per-tab file logging
-- isn't shipped with this package, and every consumer of `LC`
-- already short-circuits on nil.
...
-- Per-tab file logging is not shipped here. Set LC to nil; every
-- call site already gracefully degrades when LC is unset.
local LC = nil
