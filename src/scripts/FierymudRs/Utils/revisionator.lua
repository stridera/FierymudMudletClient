...
--
-- Adapted from demonnic/MDK 2.10.x for Mudlet script-body
-- context: upstream is `local revisionator = {...} return revisionator`
-- (loaded via require). Mudlet runs script bodies as global
-- chunks, so we expose it under FierymudRs.Utils.revisionator
-- and drop the trailing `return`. No other changes needed.
...
FierymudRs.Utils.revisionator = revisionator